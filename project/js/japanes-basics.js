  ...
  function showQuestion() {
    ...
    if (quizType === "vocab") {
      ...
      q.options.forEach(option => {
        ...
        box.onclick = () => handleAnswer(box, option.toLowerCase(), q.answer.toLowerCase());
        optionsDiv.appendChild(box);
      });
    } else if (quizType === "text") {
      questionDiv.innerHTML = q.question.replace("___", "<input id='text-input' type='text' style='font-size: 1em;'>");
      ...
      checkBox.textContent = "Antwort prüfen";
      checkBox.onclick = () => {
        let userInput = document.getElementById("audio-input").value.trim().toLowerCase();
