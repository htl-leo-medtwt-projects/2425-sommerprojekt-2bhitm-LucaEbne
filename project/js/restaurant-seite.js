...
document.getElementById("arrow-left2").addEventListener("click", () => {
    currentRestaurant = (currentRestaurant - 1 + restaurants.length) % restaurants.length;
    renderRestaurant(currentRestaurant);
});
...
function renderRestaurant(index) {
    ...
    for (let i = 0; i < restaurant.dishes.length; i++) {
        ...
        <div>
            ...
            <p class="dishPrice">Price:<br>${restaurant.dishes[i].price_yen}¥</p>
            <img class="imgFood" src="../img/Restaurant${index + 1}-Food/food${i + 1}.jpg" alt="">
        </div>
        `;
    }

    document.getElementById("cover2").src = `../img/Restaurant${index + 1}-Food/Restaurant${index + 1}.jpg`;
    document.querySelector(".headerText2").innerHTML = `
        <p>${restaurant.name}<br>${restaurant.location} – ★${restaurant.rating}</p>
    `;

    document.getElementById("description").innerText = `Öffnungszeiten: ${restaurant.opening_hours}`;
}
